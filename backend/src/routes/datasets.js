...
// GET /api/datasets/:id/trends — sales trends for a specific dataset
router.get('/:id/trends', async (req, res) => {
  try {
    const dataset_id = req.params.id;
    const Sale = mongoose.models.Sale;
    const Product = mongoose.models.Product;
    if (!Sale || !Product) return res.status(500).json({ success: false, error: 'Models not ready' });

    // Find all sales that came from this dataset
    const sales = await Sale.find({ 'metadata.dataset_id': dataset_id })
      .select('quantity price timestamp productId').lean();

    if (!sales.length) return res.json({ success: true, data: { empty: true } });

    // Daily timeseries
    const dailyMap = {};
    for (const s of sales) {
      const d = new Date(s.timestamp).toISOString().slice(0, 10);
      if (!dailyMap[d]) dailyMap[d] = { qty: 0, revenue: 0 };
      dailyMap[d].qty += s.quantity;
      dailyMap[d].revenue += s.quantity * s.price;
    }
    const timeseries = Object.entries(dailyMap)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, v]) => ({ _id: date, totalQty: v.qty, totalRevenue: +v.revenue.toFixed(2) }));
    ...
    for (const s of sales) {
      ...
      productMap[pid].qty += s.quantity;
      productMap[pid].revenue += s.quantity * s.price;
    }
    ...
    const productBreakdown = products.map(p => ({
      ...
      qty: productMap[String(p._id)]?.qty || 0,
      revenue: +(productMap[String(p._id)]?.revenue || 0).toFixed(2)
    })).sort((a, b) => b.qty - a.qty).slice(0, 10);

    // Category breakdown
    const catMap = {};
    for (const p of productBreakdown) {
      if (!catMap[p.category]) catMap[p.category] = { qty: 0, revenue: 0 };
      catMap[p.category].qty += p.qty;
      catMap[p.category].revenue += p.revenue;
    }
    const categoryBreakdown = Object.entries(catMap).map(([cat, v]) => ({
      category: cat, qty: v.qty, revenue: +v.revenue.toFixed(2)
    })).sort((a, b) => b.qty - a.qty);

    // Summary stats
    const totalQty = sales.reduce((s, r) => s + r.quantity, 0);
    const totalRevenue = sales.reduce((s, r) => s + r.quantity * r.price, 0);
    const avgPrice = sales.reduce((s, r) => s + r.price, 0) / sales.length;

    res.json({ success: true, data: {
      summary: {
        totalQty, totalRevenue: +totalRevenue.toFixed(2),
        avgPrice: +avgPrice.toFixed(2), totalSales: sales.length,
        dateRange: { from: timeseries[0]?._id, to: timeseries[timeseries.length - 1]?._id }
      },
      timeseries, productBreakdown, categoryBreakdown
    }});
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
